...
module.exports = async function (ret, database, table, object, duplicateCheck) {
    ...
    if (findTable) {
        ...
        if (duplicateCheck === true) {
            for (var index = 0; index <= latestChunk; index++) {
                const parse = JSON.parse(fs.readFileSync(`${databasePathGet()}/${database}/${table}/${keyFolder}/chunk_${index}.limebase`));
                if (parse[object[key]]) return ret({error: "duplicate"})
            }
        }
